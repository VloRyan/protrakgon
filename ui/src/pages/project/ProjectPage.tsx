...
export function ProjectPage() {
  ...
  return (
    <ItemPage error={error} isLoading={isLoading} formId={form.id}>
      ...
      <ActivitiesCard
        className="mt-2 mx-2"
        fetchOpts={{ filter: { projectId: form.getValue("id") as string } }}
      />
      <SlotCard
        className="mt-2 mx-2"
        csvFileNamePrefix={
...
const ActivitiesCard = ({ fetchOpts, ...cardProps }: ActivitiesCardProps) => {
  const [location] = useLocation();
  return (
    <Card {...cardProps}>
      <Card.Title className="text-center">
        <Row className="pt-2">
          ...
          <Col className="d-flex justify-content-center" xs="auto">
            <CreateButton size="sm" href={location + "/activity/new"} />
          </Col>

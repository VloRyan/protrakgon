...
import { EmptyFetchOpts, FetchOpts } from '@vloryan/ts-jsonapi-form/jsonapi';
import { FormsModule } from '@angular/forms';
import { MatFormField } from '@angular/material/form-field';
import { MatInput, MatLabel } from '@angular/material/input';
import { SingleObjectForm } from '@vloryan/ts-jsonapi-form/form';
import { toFilterObject } from '@vloryan/ts-jsonapi-form/functions';
    ...
    <form id="item-form" (submit)="onSubmit($event)">
        ...
        <select matNativeControl name="activityId" (input)="onInput($event)">
          ...
          @for (activity of this.activities(); track $index) {
            <option
              [value]="activity.id"
              [selected]="
                this.searchForm()?.getValue('activityId') == activity.id
              "
            >
              <fa-icon
      ...
      <mat-form-field floatLabel="always" [style.width.%]="20">
        ...
        >
          @for (comp of this.Comparators; track $index) {
            <option
              [value]="comp"
              [selected]="this.searchForm()?.getValue('fromComparator') == comp"
            >
      ...
      <mat-form-field floatLabel="always" [style.width.%]="80">
        ...
        <input
          ...
          name="from"
          type="date"
          [defaultValue]="this.asDateString(searchForm()?.getValue('from'))"
          (input)="this.onInput($event)"
        />
      </mat-form-field>
        ...
        >
          @for (comp of this.Comparators; track $index) {
            <option
              [value]="comp"
              [selected]="
                this.searchForm()?.getValue('untilComparator') == comp
              "
            >
              {{ this.ComparatorCaptions[$index] }}
            </option>
      ...
      <mat-form-field floatLabel="always" [style.width.%]="80">
        ...
        <input
          ...
          name="until"
          type="date"
          [defaultValue]="this.asDateString(searchForm()?.getValue('until'))"
          (input)="this.onInput($event)"
        />
      </mat-form-field>
      <br />
      <mat-form-field floatLabel="always" [style.width.%]="49">
        ...
        <select matNativeControl name="billable" (input)="this.onInput($event)">
          ...
          <option
            value="true"
            [selected]="searchForm()?.getValue('billable') === true"
          >
            Yes
          </option>
          <option
            value="false"
            [selected]="searchForm()?.getValue('billable') === false"
          >
            No
          </option>
        </select>
      ...
      <mat-form-field floatLabel="always" [style.width.%]="49">
        ...
        <select matNativeControl name="isOpen" (input)="this.onInput($event)">
          ...
          <option
            value="true"
            [selected]="searchForm()?.getValue('isOpen') === true"
          >
            Is open
          </option>
          <option
            value="false"
            [selected]="searchForm()?.getValue('isOpen') === false"
          >
            Is closed
          </option>
        </select>
...
export class SlotsFilterComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.searchForm.set(
      new SingleObjectForm({
        object: this.fetchOpts().filter ? this.fetchOpts().filter! : {},
      }),
    );
  ...
  onInput(ev: Event) {
    ...
    if (
      target &&
      (target.type === 'select-one' || target.type === 'text') &&
      target.value === ''
    ) {
  ...
  onSubmit(ev: Event) {
    ev.preventDefault();
    ev.stopPropagation();
    const filter = toFilterObject(
      this.searchForm()!.object as ObjectLike,
      this.queryFilterPrefix(),
    );

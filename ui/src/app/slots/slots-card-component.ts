...
import {buildQueryString, EmptyFetchOpts, FetchOpts, findInclude} from '@vloryan/ts-jsonapi-form/jsonapi';
  ...
  template: `
    <mat-sidenav-container [style.min-height.px]="500">
      <mat-sidenav #filterBar position="end" mode="push">
        <app-slots-filter-component [projectId]="this.projectId()" [fetchOpts]="this.fetchOpts()"
                                    [queryFilterPrefix]="this.queryFilterPrefix()"
                                    (filterChanged)="onFilterChanged($event,filterBar)"></app-slots-filter-component>
      </mat-sidenav>
      <mat-card appearance="outlined">
        <mat-card-header>
          <mat-card-title>Slots</mat-card-title>
          <span class="toolbar-spacer"></span>
          <a matButton="outlined" title="Download .csv file" [href]="this.csvDownloadLink()"
             [download]="'slots_'+this.projectId()+'.csv'">
            <fa-icon [icon]="['fas', 'file-download']"/>
          </a>
          <button [matButton]="wrapText()?'outlined':'filled'" title="Minimize text in columns (disable wrapping)"
                  (click)="wrapText.set(!wrapText())">
            <fa-icon [icon]="['fas', 'minimize']"/>
          </button>
          <a [routerLink]="['/project',this.projectId(),'slot','new']" matButton="outlined">
            <fa-icon [icon]="['fas', 'plus']"/>
          </a>
          <button [matButton]="this.isEmpty(this.fetchOpts().filter)?'outlined':'filled'"
          ...
          @if (isLoading()) {
            ...
            <table mat-table [dataSource]="rows()" class="results-table mat-elevation-z8">
              ...
              <ng-container matColumnDef="duration">
                ...
                <td mat-cell *matCellDef="let item">
                  {{ valueAsLocalTime(item.attributes.start) }} - {{ valueAsLocalTime(item.attributes.end) }}
                  ({{ this.formatDuration(this.calcDuration(item)) }})
                </td>
              </ng-container>
              <ng-container matColumnDef="description">
                <th mat-header-cell *matHeaderCellDef> Description</th>
                <td mat-cell *matCellDef="let item" [style.white-space]="wrapText()?' pre-wrap':''">
              ...
              <tr mat-row class="row-hover" [routerLink]="['/project',this.projectId(),'slot', item.id]"
              ...
              <tr mat-row *matRowDef="let _row; columns: ['groupHeader']; when: isGroup"
                  style="height: auto;background-color: var(--mat-sys-on-primary)"></tr>
            </table>
          }
        </mat-card-content>
      </mat-card>
...
appConfig = inject(AppConfigService)
  ...
  findActivity(object: ResourceObject) {
    ...
    let id = object.relationships && object.relationships!['activity'] ? object.relationships!['activity'].data : undefined;
    if (id == undefined) {
      return null;
    }
  ...
  protected override async loadDocument(): Promise<CollectionResourceDoc | undefined> {
    this.csvDownloadLink.set(joinPath(this.appConfig.apiUrl(),'project/' , this.projectId() , '/slot/csv') + buildQueryString(this.fetchOpts()))

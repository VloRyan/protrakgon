...
import {joinPath} from '../../../../../../ts/ts-jsonapi-form/functions';
 ...
 protected objectName :string="";
  ...
  onSubmit(ev: Event) {
    ...
    this.form()?.submit().then((doc) => {
      ...
      this.router.navigate([this.baseUrl, object.id],{replaceUrl:true}).then(
        () => {
          this.form.set(new DocumentForm({
            document: doc,
            apiUrl:  joinPath(this.baseApiUrl, object.id )
  ...
  protected refreshForm(){
    ...
    this.loadDocument().then((doc) => {
      ...
      let props = {
        document: theDoc,
        apiUrl: joinPath(this.baseApiUrl,(theDoc?.data.id ?  theDoc!.data.id : "") )
      } satisfies DocumentFormProps;
